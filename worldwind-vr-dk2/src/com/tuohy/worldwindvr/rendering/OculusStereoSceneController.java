...
import com.tuohy.worldwindvr.input.Xbox360Controller;
...
public class OculusStereoSceneController extends BasicSceneController implements StereoSceneController
{
	 ...
	 * 
	 */
	public static final double SCREEN_RENDERABLE_AREA_RATIO = 0.80;
	...
	/**
	 * This is the FOV appropriate for the Oculus Rift DK2.
	 */
	protected static final double DEFAULT_FOV = 100.0;
	...
	public OculusStereoSceneController()
	{
		...
		hmd = Hmd.create(0);
		if (null == hmd) {
			System.out.println("Could not open HMD");
		}
		else if (0 == hmd.configureTracking(ovrTrackingCap_Orientation | ovrTrackingCap_Position, 0)) {
			System.out.println("Could not configure tracking");
		}

		//old JRift implementation
	...
	protected void doDrawStereoOculus(DrawContext dc)
	{
		...
		//update the camera position based on controller inputs
		Xbox360Controller.INSTANCE.updateViewPosition(dcView);
		
		//consult the oculus for the current camera orientation
		if(hmd!=null && vrFrame.getCurrentInteractionMode()!=InteractionMode.ROBOT){
			TrackingState state = hmd.getSensorState(0);
			OvrQuaternionf orient = state.HeadPose.Pose.Orientation;
			OvrVector3f pos = state.HeadPose.Pose.Position;
			double[] pry = this.eulerAngles(orient);
			if(frameCount%20==0){
//				System.out.println(orient.x + " " + orient.y  + " " + orient.z + " " + orient.w);
//				System.out.println("not normal: " + pry[0] + " " + pry[1] + " " + pry[2]);
//				pry = this.eulerAnglesNormalized(orient);
//				System.out.println("normalized: " + pry[0] + " " + pry[1] + " " + pry[2]);
//				System.out.println(pos.x + " " + pos.y + " " + pos.z);
			}
			dcView.setPitch(Angle.fromDegrees(this.referencePitchAngleDegrees+Angle.fromRadians(pry[0]).degrees+90));
			dcView.setRoll(Angle.fromRadians(-pry[1]));
			dcView.setHeading(Angle.fromDegrees(this.referenceYawAngleDegrees - Angle.fromRadians(pry[2]).degrees));
	...
	public double[] eulerAnglesNormalized(OvrQuaternionf q) {
		double roll, pitch, yaw;
		double test = q.x * q.y + q.z * q.w;
		if (test > 0.499) { // singularity at north pole
			pitch = 2 * Math.atan2(q.x, q.w);
			yaw = (double) (Math.PI / 2);
			roll = 0;
		...
		double sqy = q.y * q.y;
		double sqz = q.z * q.z;
		pitch = Math.atan2(2 * q.y * q.w - 2 * q.x * q.z, 1 - 2 * sqy - 2 * sqz);
		yaw = Math.asin(2 * test);
		roll = Math.atan2(2 * q.x * q.w - 2 * q.y * q.z, 1 - 2 * sqx - 2 * sqz);
		return new double[]{roll, pitch, yaw};
	}
	
	public double[] eulerAngles(OvrQuaternionf q1) {
		...
		if (test > 0.499*unit) { // singularity at north pole
			...
			attitude = Math.PI/2;
			bank = 0;
			return new double[]{bank,attitude,heading};
		}
